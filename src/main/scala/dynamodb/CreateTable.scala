...
object CreateTable {
  ...
  /**
    *  Attribute describing the key schema for a table or index
    */
  ...
  case class StringAttr(name: String) extends StringType
  case class BinaryAttr(name: String) extends BinaryType
  case class NumAttr(name: String) extends NumberType
  ...
  trait HashType extends SchemaElement {
    override val keyType = KeyType.HASH
  }

  trait RangeType extends SchemaElement {
    override val keyType = KeyType.RANGE
  }
  ...
  trait DynamoTable {
    val name: String
    val attributes: Vector[Attribute]
    val partitionKey: PartitionKey
    val sortKeyOpt: Option[SortKey] = None
    val throughput: DynamoThroughput = DynamoThroughput(32, 32)
    val gsiOpt: Option[GSI] = None
    val lsiOpt: Option[LSI] = None
    val attrNames: Vector[String]
    def create(dynamoDB: DynamoDB): Table = {
      createTable(dynamoDB, name, attributes, partitionKey, sortKeyOpt, throughput, gsiOpt, lsiOpt)
    }
  }
